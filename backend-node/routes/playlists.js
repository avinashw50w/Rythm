...
router.post('/', requireAuth, (req, res) => {
    ...
    res.json({ id: result.lastInsertRowid, name, is_public: !!isPublic, track_count: 0 });
});
...
router.put('/:id', requireAuth, (req, res) => {
    ...
    if (!playlist) return res.status(404).json({ detail: 'Playlist not found' });
    if (playlist.creator_id !== req.user.id) return res.status(403).json({ detail: 'Not authorized' });

    const { name, is_public } = req.query;
    if (name) db.prepare('UPDATE playlists SET name = ? WHERE id = ?').run(name, req.params.id);
    if (is_public !== undefined) db.prepare('UPDATE playlists SET is_public = ? WHERE id = ?').run(is_public === 'true' ? 1 : 0, req.params.id);
...
router.post('/:id/tracks/:trackId', requireAuth, (req, res) => {
    ...
    const existing = db.prepare('SELECT * FROM playlist_tracks WHERE playlist_id = ? AND track_id = ?').get(req.params.id, req.params.trackId);
    if (existing) return res.status(400).json({ detail: 'Track already in playlist' });

    const countResult = db.prepare('SELECT COUNT(*) as count FROM playlist_tracks WHERE playlist_id = ?').get(req.params.id);
    const maxOrder = countResult?.count || 0;

    db.prepare('INSERT INTO playlist_tracks (playlist_id, track_id, "order") VALUES (?, ?, ?)').run(req.params.id, req.params.trackId, maxOrder);
...
router.delete('/:id/tracks/:trackId', requireAuth, (req, res) => {
    ...
    const result = db.prepare('DELETE FROM playlist_tracks WHERE playlist_id = ? AND track_id = ?').run(req.params.id, req.params.trackId);
    if (result.changes === 0) return res.status(404).json({ detail: 'Track not in playlist' });

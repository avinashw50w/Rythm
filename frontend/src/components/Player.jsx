        ...
        const handleKeyDown = (e) => {
            ...
            switch (e.code) {
                ...
                case 'ArrowUp':
                    e.preventDefault();
                    if (audioRef.current) {
                        const newVol = Math.min((audioRef.current.volume || 0) + 0.1, 1);
                ...
                case 'ArrowDown':
                    e.preventDefault();
                    if (audioRef.current) {
                        const newVol = Math.max((audioRef.current.volume || 0) - 0.1, 0);
                        audioRef.current.volume = newVol;
                        setVolume(newVol);
                    }
                    break;
                ...
                case 'ArrowRight':
                    e.preventDefault();
                    if (audioRef.current) {
                        const newTime = Math.min((audioRef.current.currentTime || 0) + 5, audioRef.current.duration || 0);
                        audioRef.current.currentTime = newTime;
                        setCurrentTime(newTime);
                    }
                    break;
    ...
    useEffect(() => {
        if (audioRef.current && !wavisRef.current) {
            try {
                ...
                // Dynamically fetch visualizers from the library
                const presets = wavisRef.current.getVisualizers();
                setVisualizerOptions(['none', 'album art', ...presets]);
            } catch (error) {
                console.error("Failed to initialize Wavis:", error);
            }
        }
    }, []);
        ...
        <>
            ...
            >
                ...
                <div className={`absolute top-8 right-8 z-20 flex gap-4 transition-opacity duration-500 ${showControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                    ...
                    <div className="relative">
                        <button
                            ...
                            className={`bg-black/40 hover:bg-white/20 p-3 rounded-full text-white backdrop-blur-md border border-white/20 hover:scale-110 transition-all cursor-pointer ${showVizMenu ? 'bg-white/20' : ''}`}
                            ...
                            <div className="absolute right-0 top-full mt-2 w-48 bg-[#181818]/90 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl overflow-hidden py-1 z-50">
                                {visualizerOptions.map((v) => (
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-3 text-sm font-bold capitalize transition-colors flex items-center justify-between cursor-pointer ${visualizerName === v ? 'text-green-400 bg-white/5' : 'text-white hover:bg-white/10'}`}
                                    >
                                        {v}
                    ...
                    <div className="relative">
                        <button
                            ...
                            className={`bg-black/40 hover:bg-white/20 p-3 rounded-full text-white backdrop-blur-md border border-white/20 hover:scale-110 transition-all cursor-pointer ${showVideoMenu ? 'bg-white/20' : ''}`}
                        ...
                        {showVideoMenu && (
                            <div className="absolute right-0 top-full mt-2 w-64 bg-[#181818]/90 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl overflow-hidden py-1 z-50 max-h-80 overflow-y-auto">
                                <button
                                    ...
                                    className={`w-full text-left px-4 py-3 text-sm font-bold transition-colors flex items-center justify-between cursor-pointer ${!currentVideo ? 'text-green-400 bg-white/5' : 'text-white hover:bg-white/10'}`}
                                >
                                    None (Black)
                                ...
                                {availableVideos.map((video) => (
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-3 text-sm font-bold transition-colors flex items-center justify-between cursor-pointer ${currentVideo === video ? 'text-green-400 bg-white/5' : 'text-white hover:bg-white/10'}`}
                                    >
                    ...
                    <button 
                        ...
                        className="bg-black/40 hover:bg-white/20 p-3 rounded-full text-white backdrop-blur-md border border-white/20 hover:scale-110 transition-all cursor-pointer"
                    >
                ...
                <div className={`absolute inset-x-0 bottom-0 pb-16 z-20 flex flex-col items-center justify-end transition-opacity duration-500 ${showControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                    <div className="text-center max-w-2xl px-8" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex items-center justify-center gap-12 backdrop-blur-md bg-black/30 p-6 rounded-full border border-white/10 shadow-2xl w-96 mx-auto">
                             <button onClick={onPrev} disabled={!hasPrev} className={`text-white/70 hover:text-white transition-transform hover:scale-110 active:scale-95 cursor-pointer ${!hasPrev && 'opacity-30'}`}>
                                <FaStepBackward size={32} />
                            </button>
                            <button
                                className="w-16 h-16 bg-white rounded-full flex items-center justify-center text-black hover:scale-110 transition-transform shadow-[0_0_30px_rgba(255,255,255,0.6)] active:scale-95 cursor-pointer"
                                onClick={onTogglePlay}
                            >
                                {isPlaying ? <FaPause size={24} /> : <FaPlay size={24} className="ml-1" />}
                            </button>
                            <button onClick={onNext} disabled={!hasNext} className={`text-white/70 hover:text-white transition-transform hover:scale-110 active:scale-95 cursor-pointer ${!hasNext && 'opacity-30'}`}>
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-black text-white px-4 h-[90px] flex items-center justify-between z-[90] border-t border-[#282828]">
                ...
                <div className="flex items-center gap-4 w-[30%] min-w-[180px]">
                    <div className="relative group w-14 h-14 bg-[#282828] rounded shadow-lg overflow-hidden flex-shrink-0 cursor-pointer" onClick={() => setIsFullScreen(true)}>
                        {currentTrack.album_art_path ? (
                        ...
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <button className="text-white hover:text-green-400 transform hover:scale-110 transition-all cursor-pointer">
                                <FaExpandAlt size={14} />
                            </button>
                        </div>
                    </div>
                    <div className="overflow-hidden min-w-0 flex flex-col justify-center">
                        <Link to={`/track/${currentTrack.id}`} className="font-medium text-sm text-white hover:underline truncate cursor-pointer">
                            {currentTrack.title}
                        </Link>
                        <Link to={`/artist/${currentTrack.artist_id}`} className="text-xs text-[#b3b3b3] truncate hover:text-white hover:underline cursor-pointer">
                            {currentTrack.artist}
                        </Link>
                    </div>
                    <button 
                        onClick={toggleFavorite} 
                        className={`ml-2 text-[#b3b3b3] hover:text-white cursor-pointer ${isFavorite ? 'text-green-500 hover:text-green-400' : ''}`}
                    >
                ...
                <div className="flex flex-col items-center max-w-[40%] w-full gap-1">
                    <div className="flex items-center gap-6 mb-1">
                        <button className="text-[#b3b3b3] hover:text-white transition-colors cursor-pointer" title="Shuffle (Coming soon)">
                        ...
                        <button 
                            className={`text-[#b3b3b3] hover:text-white transition-colors cursor-pointer ${!hasPrev ? 'opacity-30 cursor-not-allowed' : ''}`}
                            onClick={onPrev}
                            disabled={!hasPrev}
                        >
                        ...
                        <button
                            className="w-8 h-8 bg-white rounded-full flex items-center justify-center text-black hover:scale-105 transition-transform cursor-pointer"
                            onClick={onTogglePlay}
                        >
                            {isPlaying ? <FaPause size={12} /> : <FaPlay size={12} className="ml-0.5" />}
                        </button>
                        <button 
                            className={`text-[#b3b3b3] hover:text-white transition-colors cursor-pointer ${!hasNext ? 'opacity-30 cursor-not-allowed' : ''}`}
                            onClick={onNext}
                            disabled={!hasNext}
                        >
                            <FaStepForward size={20} />
                        </button>
                        <button className="text-[#b3b3b3] hover:text-white transition-colors cursor-pointer" title="Repeat (Coming soon)">
                ...
                <div className="flex items-center justify-end gap-2 w-[30%]">
                    <div className="flex items-center gap-2 w-32 group">
                        <button onClick={() => setVolume(volume === 0 ? 1 : 0)} className="text-[#b3b3b3] hover:text-white cursor-pointer">
